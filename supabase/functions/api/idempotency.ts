/**
 * Helpers de idempotência para POST /registros.
 * A chave é gravada no metadado: IDEM=<uuid>
 */
...
export function findRowByIdempotencyKey(
  rows: string[][],
  idemKey: string
): { rowNumber: number; id: string; cidade: string; instrumento: string; ministerio: string; musica: string; metadado: string; horario: string } | null {
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i] || [];
    const metadado = String(row[7] ?? "");
    if (extractIdemFromMetadado(metadado) === idemKey) {
      return {
        rowNumber: i + 2, // sheet starts at row 2 with header on 1; values from A2
